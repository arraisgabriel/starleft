...
> **⚠ CANONICAL — ZECA'S SECRET [PLANNED] (seed early, reveal late; Zeca does NOT know yet):** Zeca was raised an orphan as **Dell Tusk's personal special intern / chore-boy**. Tusk wired him with **bespoke implants** that grant his Overclock *and* **A&O internal access** — the in-fiction reason his Overclock cracks Upstream Custody in XIX (his credentials are baked into his cyberware; the vault still recognises him as *inside*). The buried twist: **Zeca is Tusk's bastard son**, and **Tusk murdered Zeca's mother** to hide the boy. This makes the interlock literal **patricide-by-data** — the son holds the only copy of the father. It is a *monstrous* beat for Tusk, never a sympathetic one (he never gets pitied). Full canon + reveal cadence: `docs/story-next-steps-ceo-arc.md` §3 HERO 2.
...
- **Zeca:** `starleft-unit-forge` near-defenceless Intern hero; **OVERCLOCK** aura scaling the per-`dt` increment up at the verified timer sites — build `js/core.js:25` (`b.buildProg += dt`) and crew-assist `js/units.js:966` (mind `ASSIST_BUILD_RATE`, `js/config.js:6`), production `js/core.js:53` (`b.prodTime += dt`), gather `js/units.js:942` (`u.gatherTimer += dt`) — accelerate the increment, never reduce a `buildTime`; **CRUNCH** active burst (cooldown-gated). `cfg.captives` hero entry; Nino-proximity rescue is zero-code (`freeCaptives`, `js/core.js:221`, `heroId==='Nino'` at `:225`). Dossier + barks + TTS. **Canon (carry into his dossier/iconics, kept hidden until the finale):** Zeca is Tusk's orphan intern/chore-boy with Tusk-given implants → A&O internal access, and secretly Tusk's **bastard son** (mother murdered to hide him). See the interlock callout above + `docs/story-next-steps-ceo-arc.md` §3 HERO 2.
