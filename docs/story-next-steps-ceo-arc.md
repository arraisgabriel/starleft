...
- **Reborn is NOT a reward of this arc.** Resurrection is already live. Arc 3 adds **no gate, no freeze,
  no unlock**. Its resurrection stakes are about Voss **purging the backup *pool*** the dead are written
...
- **Tone stays Arc-canon:** dark, devastated, Hades-not-Marvel, satirical, **never redemptive or
  triumphant**. Victories are hollow; Voss is never pitied; the heroes' arrival is grim relief, not hope.
...
### Voss's plan — PROJECT CHAPTER ELEVEN (three planks)

| Plank | What Voss does | What strips it |
|-------|----------------|----------------|
...
| **2. PURGE** | A countdown that erases the upstream backup pool, so the memorial's dead can never be written home again. | **ZÉ's Overclock (XVII–XIX)** — the only thing fast enough to out-race the clock; her vault heist seizes the custody keys and saves the pool. |
| **3. IPO** | Relaunch the cure for death as a Voss-only subscription, forever. | **The finale (XXII–XXIII)** — raze the keynote, liquidate the man inside REX, take the IPO and find it hollow. |

### Why the heroes are *literally* the keys (the interlock)

This is the answer to "make the recruitments matter, not be detours":

- **Voss is a GRAAL backup** (§2) → killing him normally just **re-instantiates** him. The custody keys
  **Zé** seizes in XIX **hold Voss's own backup** → owning them is the only way to make him stay dead.
  **Zé is what makes Voss killable.**
...
- Remove either: no Zé → the dead are purged and Voss is immortal; no Rust → the finale is unwinnable.
...
expensed as a "depreciated asset", Zé leased as "velocity") → enforcers (the `.5` interludes) → the
building that walks (REX/Voss, XXIII). The connective tissue: ***A&O eats its own stars; you recruit the
leftovers.***

---

## 2. Villain spec — DELLAN "DELL" VOSS
...
might pity (true to world-bible §3: A&O is faceless capital incarnate; Voss is the first time it wears a
face, and the face is a lie).

**The twist (locked).** Voss is the **first successful GRAAL write** — a backup of A&O's long-churned
...
`js/config.js`) and makes Voss the **player's dark mirror**: you both run the same machine; only the price
...
- **XVI** — Rust's insider knowledge plants it: he scanned Voss's face for a marketing **render** and the
...
Voss's escape vehicle**. Verified: `finaleVillainIndex()` (`js/villains.js:838`) and the finale branch of
...
rewrite**. This is a crawl/taunt re-skin plus (optionally) a bespoke Voss-in-cockpit sprite — see §6.
...
### HERO 1 — HOLT "RUST" ARMAND · Mechfounder · introduced **Ep XV**
...
- **Recruitment — boss-duel → defection.** A&O fields him *against* you in XV as its contracted
  ...
  unit). You out-duel him; the fight ends not in death but in him reading his own file: Voss booked him a
...
- **Voice — gallows-corporate; the arc's tonal relief, never hope-core** (lore-forge seeds):
  ...
  - On Voss: "I built the face he's wearing. Trust me, the warranty's expired."
...
### HERO 2 — MEI "ZÉ" OKONKWO · Gifted Intern · introduced **Ep XVII**
...
- **Recruitment — Nino-proximity rescue** (the Biba pattern, a `cfg.captives` entry with `hero:true`).
  Held in an A&O "growth" sweatshop, kept because she is *faster than the algorithm* — her throughput is
  what makes the purge clock viable in the first place. **Verified:** `freeCaptives()` hardcodes Nino as
  the only rescuer (`js/core.js:215`) and Nino is carried by XVII → **zero new code if Nino reaches her.**
...
- **Kit — "speeds up the game a lot" = Overclock aura + Crunch burst** (the author's two-part ask):
  - **Passive OVERCLOCK aura (~6 tiles):** accelerates tempo by scaling the **per-`dt` increment UP** at
    ...
    (`js/units.js:886,888`). Economy *and* army tempo speed up around her. **(These timers INCREMENT —
  ...
  - She inherits worker-tier `dmg` (≈4) — **near-defenceless on purpose**, so keeping her alive is the
    tension. She is a **non-combat hero**; her power is the clock, not the gun.
...
- **Finale stake:** her Overclock cracks the vault keys (XIX), out-builds the manual purge (XX), and is the
  reason the dead survive Voss's death. No Zé → no keys → Voss is immortal and the pool is ash.
...
|----|-------|---------------|-------|-----|------|----------------|
| **XIV** | THE RECALL NOTICE | Asset Recovery & Reconciliation | tech (crater / HUB perimeter) | CEO | Voss surfaces, names himself, serves **CHAPTER ELEVEN** — company + dead declared delinquent inventory; the **purge countdown** on the backups begins. Survive the first repo wave. | — |
| **XV** | THE FOUNDRY RAID | Fabrication & Founder Programs | tech + coolant seas | Mechfounder | Raid the mech foundry stamping out foreclosure-mechs → strips Voss's army. You're a raider now. | **Duel RUST** |
| **XV.5** | THE NON-COMPETE | Legal Enforcement (Founder-warden) | tech (contract-arena slab) | interlude | Voss serves a non-compete; a Founder-warden hunts Rust — "company property doesn't quit." | **Rust defects** (post-duel `captiveHero` spawn). `gateAfter:14 returnTo:15`, `tower_guardian` base. |
| **XVI** | THE SEVERANCE PACKAGE | Repossession & Recovery | tech | Mechfounder | A&O comes in force to repossess its "depreciated asset"; defend Rust, turn his foundry intel on A&O. Rust hints Voss is a "render." | Rust fully aboard; Stomp/Recall online. |
| **XVII** | THE GROWTH TEAM | Growth & Velocity (Intern sweatshop) | tech (open-plan, Acquihire register) | Intern | Free the engine of A&O's cruelty to make it yours — grim, not heroic. | **Rescue ZÉ** (Nino-proximity). Overclock previews. |
| **XVII.5** | THE PERFORMANCE REVIEW | People Ops (Enforcer-hunter) | tech + ice (cold review-room) | interlude | A&O sends a hunter to "review" Zé's departure; shield the near-defenceless hero while finishing it. | `gateAfter:16 returnTo:17`, `ao_enforcer` base. |
...
| **XIX** | THE DATA VAULT | Upstream Custody (backup archive) | tech + ice (deep cold-storage) | Intern | Decisive heist: **Zé's Overclock cracks the vault in minutes**; seize the custody keys → backup pool saved. The keys also hold **Voss's own backup** — now he can die for good. Hollow win: the dead are yours as data; the Wake still writes only 3. | — |
| **XX** | THE BURN RATE | Risk & Liquidation | tech + grass (HUB approaches) | CEO | Voss triggers the **manual purge** and comes for the keys. Starved survive/escort — get the keys home. Both heroes load-bearing. | — |
| **XXI** | THE OFFSITE | Orbital & Continuity Division | tech + ice (frozen spaceport) | CEO | Raze Voss's off-world **soul-server** launch site so he can't flee his own death — he must field himself. Hard Musk-satire beat. | — |
| **XXII** | THE GOLDEN PARACHUTE | Office of the Managing Partner | tech (bunker-campus + keynote stage) | CEO | Stripped of foundry / vault / launch, Voss has one asset: the IPO keynote. Raze it under the purge clock; as his HQ falls he **flees into REX**. Full Voss reveal. | `heroesAlive` quest (NINO, BIBA, RUST, ZÉ). |
| **XXIII** | THE LIQUIDATION EVENT | REX (Voss inside it) | tech (REX arena) | CEO finale | Liquidate the man inside the foreclosure-that-walks. **One** REX (foundry gone), survivable (Rust's chassis), Voss permanently killable (you hold his backup). No triumph — the foreclosure foreclosed; the IPO is yours and hollow. | REX `finale:true` routing 1:1, crawl re-skinned. |
...
- **Vars:** `{party}`, `{fallen}`, `{biba}`. **Memorial:** the purge clock = names that can *no longer be
  written home* if Voss wins (Reborn itself stays fully usable in the HUB throughout).

### XV — THE FOUNDRY RAID · win-verb: `razeAll` (+ embedded boss) · tech + coolant seas
> Voss does not build soldiers; he stamps them. One foundry on the coolant coast prints the foreclosure —
...
> render since the year he supposedly died. Defend your new asset. Start asking what Voss actually is.
- **Reveal:** first seed that **Voss re-instantiates.**
...
> the algorithm can bill — so they never let her leave, and her throughput is the engine zeroing your dead.
> Walk Nino through the open plan to the desk they chained her to. Free the fastest hands in the wasteland
> and point them at the clock.
- **Hook:** **Rescue Zé** (Nino-proximity, Biba pattern). **Career:** Zé joins the hero track; Overclock
  previews on the walk out.

### XVII.5 — THE PERFORMANCE REVIEW · win-verb: `defeatVillain` (protect Zé) · tech + ice · `gateAfter:16 returnTo:17`
> Attrition has a department. A&O dispatches a reviewer to mark Zé's departure as a performance issue,
> permanently. She cannot fight — she can only run the clock. Keep her alive in the cold while you close
> the reviewer's file for good.
- **Career:** Zé's first life-event (priced-at-exposure trauma).

### XVIII — THE VELOCITY TRAP · win-verb: `survive`/`winBy` race · tech + ice
> Voss read the same logs you did and moved the purge up a quarter. The only counter to his speed is yours.
...
> The dead are not in the ground; they are in a vault, encrypted, booked as inventory. Zé cracks Upstream
...
> the keys to the man. Take them both. You hold the wall now; you also hold the only copy of Voss that can
> ever be deleted.
- **Pivot:** the dead are **saved from the purge**; **Voss becomes mortal.** Reborn untouched — the *pool*
  is now safe.

### XX — THE BURN RATE · win-verb: `survive`/`escort` (starved economy) · tech + grass (HUB approaches)
> Stripped of his foundry and his vault, Voss stops sending departments and triggers the purge by hand —
...
> Voss always had an exit: a launch site on the ice, soul-servers fueled for orbit, a backup of himself
...
> One asset left: the keynote. Voss takes the stage to IPO the cure for death as a subscription with no
...
- **Reveal:** full Voss. **Quest:** `heroesAlive` (NINO, BIBA, RUST, ZÉ).
...
> they threw away is standing in front of yours. Liquidate Dell Voss. Take the IPO. Ring the bell in an
...
2. **`[ENG]` Vet-carry cap.** `vetCarryCountFor(idx)=2+floor((idx-1)/2)` (`js/career.js:195`) is unbounded
   ...
   read the default). Heroes already bypass the cap (`carryoverHeroes[]`), so Rust/Zé don't count.
3. **`[ENG]` Overclock aura + Crunch burst (Zé).** Per-frame radius scan scaling the per-`dt` increment at
...
6. **`[ENG]` Two `heroAura()` lines** (`js/render.js:1426`): Rust `[255,140,60]`, Zé teal. *Trivial.*
7. **`[MAP]` The 12 map configs** — crawls, summaries, objectives, quests (all shipped quest types),
   ...
   `ao_enforcer` ids), Rust/Zé `cfg.captives`/`captiveHero` declarations, and the **Voss-in-REX crawl
...
8. **`[ART/VOICE]` Sprites** — Voss (keynote + in-REX-cockpit), Rust as boss + as hero Founder Mech, Zé as
...
9. **`[ART/VOICE]` Dossiers + voice lines** — fixed dossiers for Rust & Zé, life-events (depreciated-asset;
   priced-at-exposure), selection barks, and Voss's keynote taunts → `starleft-lore-forge` (append-only
...
- **Voss is never redeemed or pitied.** He is capital wearing a face; his defeat is a write-off. Do not
...
- **Rust's humour is gallows, not hope-core.** Jokes about dental, vesting, foreclosure — relief, never
  optimism.
- **Zé's speed is grief with a stopwatch.** Her gift is a wound; the satire is that capital turned a
...
| 2 | CEO identity | **Dellan "Dell" Voss**, the **first successful GRAAL write** (re-instantiating backup). |
| 3 | Intern power | **Overclock aura + active Crunch burst.** |
| 4 | Reborn-Cyborg | **Already live; MUST NOT be gated/postponed.** Stakes = Voss purging the backup *pool*. |

**Still open for whoever builds first (recommendations in §3/§6):** exact Crunch cooldown/strength
numbers; whether XV's duel and XV.5's defection are one map or two; bespoke Voss-in-REX sprite vs.
